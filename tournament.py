...
def enterTournament(name, tournament):
    ...
    if not matches:
        cursor.execute(
            "UPDATE players SET tournament = %s WHERE name = %s",
            (tournament_id, name,))
        db.commit()
    else:
        print (
...
def leaveTournament(name):
    ...
    db, cursor = connect()
    cursor.execute(
        "UPDATE players SET tournament = NULL WHERE name = %s",
        (name,))
    db.commit()
...
def swissPairings(tournament):
    ...
    while wasSkipped(odd_player, tournament_id):
        ...
        pairings_swiss = make_unique(pairings_all, tournament_id)
        last_player = odd_player
        odd_player = findOdd(pairings_swiss, tournament_id)
        #if the odd_player still the same
        if last_player == odd_player:
            print (
                "\nThere is no way to continue the matches with odd number "
                "of players (unpaired player can receive free win only once")
            pairings_swiss = []
            break
